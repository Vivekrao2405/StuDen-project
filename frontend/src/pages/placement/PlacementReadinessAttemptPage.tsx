...
import {
  getPlacementReadinessAttempt,
  savePlacementReadinessAnswer,
  submitPlacementReadinessAttempt,
} from "@/lib/api/endpoints/placement";
import type { PlacementAttemptDetailResponse, PlacementAttemptQuestionView } from "@/lib/api/placementTypes";
...
/** The readiness-assessment taking experience. Mirrors AssessmentTakingPage's structure exactly
 * (same option list/progress/nav components, same autosave-then-submit flow) — the two engines
 * differ only in data shape (role/skill breakdown vs single-skill topic breakdown), not in UX. */
export function PlacementReadinessAttemptPage() {
  const { attemptId = "" } = useParams<{ attemptId: string }>();
  const navigate = useNavigate();
  const toast = useToast();
  const { data, error, loading, refetch } = useAsync(() => getPlacementReadinessAttempt(attemptId), [attemptId]);
  ...
  const detail = data && data.status === "IN_PROGRESS" ? (data as PlacementAttemptDetailResponse) : null;

  useEffect(() => {
    if (!data) return;
    if (data.status !== "IN_PROGRESS" || !("remainingSeconds" in data)) {
      navigate(ROUTES.placementReadinessResult(attemptId), { replace: true });
      return;
    }
  ...
  function handleSelect(question: PlacementAttemptQuestionView, selectedOptionIds: string[]) {
    setAnswers((prev) => ({ ...prev, [question.id]: selectedOptionIds }));
    setSaveStates((prev) => ({ ...prev, [question.id]: "saving" }));
    savePlacementReadinessAnswer(attemptId, question.id, selectedOptionIds)
  ...
  async function handleSubmit() {
    setSubmitting(true);
    try {
      await submitPlacementReadinessAttempt(attemptId);
      navigate(ROUTES.placementReadinessResult(attemptId), { replace: true });
    } catch (err) {
  ...
  if (error || !data || !detail) {
    return (
      <ErrorState
        ...
        message={error?.message ?? "This readiness assessment isn't available."}
        onRetry={refetch}
      />
    );
  }
    ...
    <div className="mx-auto max-w-2xl space-y-5 px-4 pt-6 pb-[calc(4rem+env(safe-area-inset-bottom)+1.5rem)] sm:px-0 lg:pb-6">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="truncate text-xs font-medium text-muted-foreground">{detail.roleName} Readiness</p>
          <h1 className="truncate text-lg font-bold text-foreground">{detail.assessmentTitle}</h1>
        </div>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Submit readiness assessment?</DialogTitle>
            <DialogDescription>
